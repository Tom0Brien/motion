...
TEST_CASE("Test computing next foot step", "[MotionGeneration]") {
    ...
    SECTION("Forward walk command") {
        ...
        auto foot_placement                       = walk_engine.compute_next_footstep(walk_command);
        REQUIRE(foot_placement.isApprox(expected_foot_placement));
    }
}
...
TEST_CASE("Test generating swing foot trajectory", "[MotionGeneration]") {
    ...
    SECTION("Forward walk command") {
        walk_command << 1.0, 0.0, 0.0;
        walk_engine.generate_swingfoot_trajectory(walk_command);
